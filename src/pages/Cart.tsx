...
const Cart = () => {
 ...
 // 💡 FUNCIÓN MODIFICADA: Creación de la Orden
 const handleConfirmCheckout = (data: GuestCheckoutFormData) => {
  
  // 1. OBTENER DATOS FINALES DEL CARRITO
  const currentCartItems = getCart();
    const subtotal = getCartTotal(); // Subtotal es el total sin aplicar descuentos extra o envío
  ...
  if (currentCartItems.length === 0) {
   ...
   return;
    ...
    // 3. CONSTRUIR EL OBJETO DE ORDEN COMPLETO
    const newOrder: Omit<Order, 'id'> = {
        ...
        // Métricas y Estado (Valores iniciales post-checkout)
        paymentId: 'PAY-' + Date.now().toString().slice(-6), 
        statePago: 'Pendiente', // El pago debe estar Pendiente hasta que se confirme
        Tracking: 'PENDIENTE', 
